...
LogJam is designed to import rails log files from another rails application. The standard rails log files don't contain as much information as we'd like, so we offer you some choices of what to do in the application you want to study:

===Leave your application as it is

Note that this will not work if you are running more than one server process, unless you have modified your application to produce syslog compatible log files. If you want to use LogJam with standard Rails log files, then modify this section of config/initializers/matchers.rb to enable the RAILS_LOG_LINE_SPLITTER instead of the SYSLOG_LINE_SPLITTER.
...
If you are running more than one server process, you must modify the log lines to include the process_id and host. 
LogJam itself is an example of how to do this. You can copy lib/hodel_3000_compliant_logger.rb into your own app, as well as these lines from environment.rb:
...
If your application sets the global variable $user_id then Hodel3000CompliantLogger#format_message will also log this info. One easy way to do this is to add a before_filter to your ApplicationController that looks something like this:
...
To configure time_bandits to log memcache or GC info, copy config/initializers/time_bandits.rb from LogJam into your application.
